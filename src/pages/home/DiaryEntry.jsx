        ...
        <div className="basis-4/5 border border-red-500 p-4">
            <form onSubmit={handleSubmitClick}>
                <div className="flex mt-4">
                    <label className="basis-1/5 custom-display-label" for="summaryInput">Summary</label>
                    <div className="basis-4/5">
                        <input type='text' disabled={viewOnly} name="summary" id="summaryInput" onChange={handleInputChange} className={`custom-input ${errorData?.summary ? "red-border" : ""} ${viewOnly ? "input-disabled" : ""}`} value={formData.summary} />                            
                        <div className="custom-error">{errorData.summary ? errorData.summary : null}</div>
                    </div>
                </div>
                <div className="flex mt-4">
                    <label className="basis-1/5 custom-display-label" for="dateInput">Date</label>
                    <div className="basis-4/5">
                        <input type='date' disabled={viewOnly} id="dateInput" name="currentDate" onChange={handleInputChange} className={`custom-input ${errorData?.currentDate ? "red-border" : ""} ${viewOnly ? "input-disabled" : ""}`} value={formData.currentDate} />                            
                        <div className="custom-error">{errorData.currentDate ? errorData.currentDate : null}</div>
                    </div>
                </div>
                <div className="flex mt-4">
                    <label className="basis-1/5 custom-display-label" for="activityInput">Activity</label>
                    <div className="basis-4/5">
                        <div className="flex">
                            <textarea name="activity" id="activityInput" disabled={viewOnly} onChange={handleInputChange} className={`custom-input ${errorData?.activity ? "red-border" : ""} ${viewOnly ? "input-disabled" : ""}`} value={formData.activity} />              
                ...
                <div className="flex mt-4">
                    <label className="basis-1/5 custom-display-label" for="intensityInput">Intensity Rating</label>
                    <div className="basis-4/5">
                        <div className="flex">
                            <Rating disabled={viewOnly} id="intensityInput" value={formData.intensityRating} onChange={ratingOnChange}></Rating>
                        </div>
                    </div>
                </div>
